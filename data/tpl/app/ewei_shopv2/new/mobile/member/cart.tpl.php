...
<script type="text/html" id="tpl_member_cart">
	...
	<img src="<?php  echo EWEI_SHOPV2_LOCAL?>static/new/images/requestEmptyImage.png" style="width: 6rem;margin-bottom: .5rem;"><br/><p style="color: #999;font-size: .75rem">您还没有任何订单</p><br/>
	...
	<span class="refresh-icon"><a href="<?php  echo mobileUrl()?>" style="display: block;color: #c2a674">再去逛逛</a></span>
			...
			>
				...
				<div class="fui-list-inner">
					...
					<div class='price'>
						...
						<span style="color: #c2a674">&yen;</span>
						<span class="marketprice" style="color: #c2a674"><%g.marketprice%></span>
...
<script type="text/html" id="tpl_member_cart_footer">
	...
	<div class="fui-footer cartmode" style="bottom: 2.45rem">
		<div class="fui-list noclick">
			...
			<div class="fui-list-inner">
				<div class='subtitle' >合计:<span class="text-danger bigprice" style="color: #c2a674;"> ￥</span><span class='text-danger totalprice  bigprice' style="color: #c2a674;"><%totalprice%></span></div>
				<!--<div class='text'>不含运费</div>-->
			</div>
			<div class='fui-list-angle'>
				<div style="background-color: #c2a674;	width: 5rem;" class="btn  btn-submit <%if total<=0%>}btn-default disabled<%else%>btn-danger<%/if%>" <%if total<=0%>stop="1"<%/if%>>立即预约(<span class='total'><%total%></span>)</div>
		</div>
	</div>
	</div>
	<div class="fui-footer editmode  <?php  if(is_weixin()) { ?><%if height == 2436 && width == 1125%>iphonex<%/if%><?php  } ?>" style="bottom: 2.45rem">
		<div class="fui-list noclick">
			<div class="fui-list-media">
				<label class="checkbox-inline editcheckall"><input type="checkbox" name="checkbox" class="check goods-check goodsCheck" />&nbsp;全选</label>
			</div>
			<div class="fui-list-inner"></div>

			<div class='fui-list-angle'>
				<div class="btn  btn-default btn-favorite disabled attention">移到收藏</div>
