...
class Apply_EweiShopV2Page extends CommissionMobileLoginPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else if ($type == 3) //银行卡提现
			{
				$realname = trim($_GPC['realname']);
                $realId = trim($_GPC['realid']);//身份证号
                $bankname = trim($_GPC['bankname']) ? trim($_GPC['bankname']):'';
				$bankcard = trim($_GPC['bankcard']);
				$bankcard1 = trim($_GPC['bankcard1']);
                $bankPhone = trim($_GPC['bankphone']);
				if (empty($realname)) 
				{
//					show_json(0, '请填写姓名!');
                    return $this->message('请填写姓名!', mobileUrl('commission/apply'), 'info');

                }
                if (empty($realId))
                {
                    return $this->message('请填写身份证号!', mobileUrl('commission/apply'), 'info');
                }

//				if (empty($bankname))
//				{
////					show_json(0, '请选择银行!');
//                    return $this->message('请选择银行!', mobileUrl('commission/apply'), 'info');
//
//                }
				if (empty($bankcard)) 
				{
//					show_json(0, '请填写银行卡号!');
                    return $this->message('请填写银行卡号!', mobileUrl('commission/apply'), 'info');

                }
//				if (empty($bankcard1))
//				{
////					show_json(0, '请填写确认卡号!');
//                    return $this->message('请填写确认卡号!', mobileUrl('commission/apply'), 'info');
//
//                }
//				if ($bankcard != $bankcard1)
//				{
////					show_json(0, '银行卡号与确认卡号不一致!');
//                    return $this->message('银行卡号与确认卡号不一致!', mobileUrl('commission/apply'), 'info');
//
//                }
                if (empty($bankPhone))
                {
                    return $this->message('请填写预留手机号!', mobileUrl('commission/apply'), 'info');
                }
				$apply['realname'] = $realname;
                $apply['realid'] = $realId;
                $apply['bankname'] = $bankname;
                $apply['bankcard'] = $bankcard;
				$apply['bankphone'] = $bankPhone;

				$res = $this->checkBank($apply['bankcard'],$apply['realid'],$apply['bankphone'],$apply['realname']);
				$res = json_decode($res,true);
//				var_dump($res['status']);exit();

				//验证不通过
				if ($res['status'] != '01'){
                    return $this->message($res['msg'], mobileUrl('commission/apply'), 'info');
                }

                if ($apply['bankname'] != $res['bank']){
                    $apply['bankname'] = $res['bank'];
                }
...
//			show_json(1, '已提交,请等待审核!');
            return $this->message('已提交,请等待审核!', mobileUrl('member/index'), 'info');
     ...
     *  accountNo STRING	银行卡号
        ...
        name	  STRING	姓名
     * */
	private function checkBank($accountNo,$idCard,$mobile,$name){
        $host = "https://bankali.market.alicloudapi.com";
        $path = "/bank4RealCheck";
        $method = "GET";
        $appcode = "c9d7deef138647d39d5475a03277a55c";
        $headers = array();
