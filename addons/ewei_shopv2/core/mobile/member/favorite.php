...
class Favorite_EweiShopV2Page extends MobileLoginPage
{
	...
	public function get_list()
	{
		...
		if (!empty($total)) {
			$sql = 'SELECT f.id,f.goodsid,g.title,g.subtitle,g.thumb,g.marketprice,g.productprice,g.merchid FROM ' . tablename('ewei_shop_member_favorite') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
