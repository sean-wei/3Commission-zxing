...
class Category_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		...
		$goosList = $this->getGoods($pcate);

		//默认第一个分类
		if ($pcate == null){
            $pcate = $category['parent'][0][0]['id'];
            $goosList = $this->getGoods($pcate);
        }
...
//		var_dump($category);exit();
	...
	//获取商品
    protected function getGoods($pcate){
		if ($pcate != null && $pcate !=''){
            $goods = pdo_fetchall('select * from ' . tablename('ewei_shop_goods') . ' where status=1 and pcate=' . $pcate . ' order by displayorder desc');
//			var_dump($goods);exit();
			foreach ($goods as &$item){
				$item['thumb'] =  IMAGE_PATH . $item['thumb'];
			}
			unset($item);
        }
