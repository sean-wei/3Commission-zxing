...
class Detail_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		if (
            //记次时商品可加入购物车
//		    ($goods['isverify'] == 2) ||
            ($goods['type'] == 2) || ($goods['type'] == 3) || ($goods['type'] == 20) || !(empty($goods['cannotrefund'])) || !(empty($is_task_goods)) || !(empty($gifts)))
		{
			$canAddCart = false;
		}
		...
		if ($com) 
		{
			...
			if ($goods['nocommission'] == 0) 
			{
                ...
                $goods['commissionRate'] = $this->getCommissionRate($goods, $glevel, $cset);
	...
	public function get_code() 
	{
        ...
        //服务器中文乱码
        $press = mb_convert_encoding('长按二维码扫码购买', "html-entities", "utf-8");
        ...
        if (com('goodscode'))
		{
			...
			if ($commission_data['codeShare'] == 1) 
			{
                ...
                $codedata = array( 'portrait' => array('thumb' => (tomedia($_W['shopset']['shop']['logo']) ? tomedia($_W['shopset']['shop']['logo']) : tomedia($member['avatar'])), 'left' => 40, 'top' => 40, 'width' => 100, 'height' => 100), 'shopname' => array('text' => mb_convert_encoding($_W['shopset']['shop']['name'], "html-entities", "utf-8"), 'left' => 160, 'top' => 80, 'size' => 28, 'width' => 360, 'height' => 50, 'color' => '#333'), 'thumb' => array('thumb' => tomedia($goods['thumb']), 'left' => 40, 'top' => 160, 'width' => 560, 'height' => 560), 'qrcode' => array('thumb' => tomedia($qrcode), 'left' => 23, 'top' => 730, 'width' => 220, 'height' => 220), 'title' => array('text' => mb_convert_encoding($title, "html-entities", "utf-8"), 'left' => 230, 'top' => 770, 'size' => 24, 'width' => 360, 'height' => 50, 'color' => '#333'), 'price' => array('text' => mb_convert_encoding($price, "html-entities", "utf-8"), 'left' => 270, 'top' => 880, 'size' => 30, 'color' => '#f20'), 'desc' => array('text' => $press, 'left' => 210, 'top' => 980, 'size' => 18, 'color' => '#666') );
			}
			else if ($commission_data['codeShare'] == 2) 
			{
				...
				$title = '    ' . $title[0] . "\r\n" . '    ' . $title[1];
				$codedata = array( 'thumb' => array('thumb' => tomedia($goods['thumb']), 'left' => 20, 'top' => 20, 'width' => 150, 'height' => 150), 'title' => array('text' => mb_convert_encoding($title, "html-entities", "utf-8") , 'left' => 170, 'top' => 30, 'size' => 22, 'width' => 430, 'height' => 90, 'color' => '#333'), 'price' => array('text' => mb_convert_encoding($price, "html-entities", "utf-8") , 'left' => 210, 'top' => 120, 'size' => 30, 'color' => '#f20'), 'qrcode' => array('thumb' => tomedia($qrcode), 'left' => 170, 'top' => 200, 'width' => 300, 'height' => 300), 'desc' => array('text' => $press, 'left' => 205, 'top' => 510, 'size' => 18, 'color' => '#666'), 'shopname' => array('text' => mb_convert_encoding($_W['shopset']['shop']['name'], "html-entities", "utf-8") , 'left' => 0, 'top' => 585, 'size' => 28, 'width' => 640, 'height' => 50, 'color' => '#fff') );
			}
			else if ($commission_data['codeShare'] == 3) 
			{
				...
				$title = '                ' . $title[0] . "\r\n" . '                ' . $title[1];
				$codedata = array( 'title' => array('text' => mb_convert_encoding($title, "html-entities", "utf-8"), 'left' => 27, 'top' => 40, 'size' => 22, 'width' => 600, 'height' => 90, 'color' => '#333'), 'thumb' => array('thumb' => tomedia($goods['thumb']), 'left' => 0, 'top' => 150, 'width' => 640, 'height' => 640), 'qrcode' => array('thumb' => tomedia($qrcode), 'left' => 20, 'top' => 810, 'width' => 220, 'height' => 220), 'price' => array('text' => mb_convert_encoding($price, "html-entities", "utf-8") , 'left' => 280, 'top' => 870, 'size' => 30, 'color' => '#000'), 'desc' => array('text' => $press, 'left' => 280, 'top' => 950, 'size' => 18, 'color' => '#666') );
			}
	...
	//获取佣金比例
    public function getCommissionRate($goods, $level, $set)
    {
        global $_W;
        $commission = 0;
        $commissionRate = 0;
        ...
        if ($goods['hascommission'] == 1)
        {
            ...
            {
                $commission = ((1 <= $set['level'] ? ((0 < $goods['commission1_rate'] ? ($goods['commission1_rate'] * $goods['marketprice']) / 100 : $goods['commission1_pay'])) : 0));
                $commissionRate = $goods['commission1_rate'];
            }
            else
            {
                $price_all = array();
        ...
        {
            $commission = ((1 <= $set['level'] ? round(($level['commission1'] * $goods['marketprice']) / 100, 2) : 0));
            $commissionRate = round($level['commission1']);
        }
        else
        {
            $commission = ((1 <= $set['level'] ? round(($set['commission1'] * $goods['marketprice']) / 100, 2) : 0));
