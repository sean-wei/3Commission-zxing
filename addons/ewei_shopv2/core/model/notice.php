				...
				else if ($order['status'] == 3) 
				{
					...
					if ($pv && !(empty($order['virtual']))) 
					{
						if (empty($usernotice['virtualsend'])) 
						{
							...
							$insert = array('openid' => $openid, 'tag' => 'virtualsend', 'default' => serialize($msg), 'cusdefault' => $text, 'orderid' => $orderid, 'url' => $url, 'datas' => serialize($datas), 'appurl' => $appurl, 'createtime' => time(), 'sendtime' => time() + 60, 'uniacid' => $_W['uniacid']);
